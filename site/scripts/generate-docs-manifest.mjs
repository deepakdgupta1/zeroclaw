...
function detectAudience(relativePath, journey) {
    ...
    if (journey === "contribute" || journey === "localize")
        return "contributor";
...
function inferTags(
    ...
    const rules = [
        {
            pattern:
                /(getting-started|one-click-bootstrap|onboard|setup|readme)/,
            tag: "onboarding",
        },
        {
            pattern: /(docker|network|gateway|deploy|daemon)/,
            tag: "deployment",
        },
        {
            pattern: /(security|sandbox|advisory|vulnerability|audit)/,
            tag: "security",
        },
        {
            pattern: /(commands|config|providers|channels|reference|schema)/,
            tag: "reference",
        },
        {
            pattern: /(operations|runbook|sop|release|ci|workflow|gate)/,
            tag: "operations",
        },
        {
            pattern:
                /(langgraph|matrix|mattermost|nextcloud|qwen|glm|custom-provider)/,
            tag: "integrations",
        },
        {
            pattern: /(hardware|arduino|esp32|nucleo|datasheet)/,
            tag: "hardware",
        },
        { pattern: /(i18n|translation|locale)/, tag: "i18n" },
        {
            pattern: /(contributing|reviewer|pull request|pr-workflow|project)/,
            tag: "contributing",
        },
        {
            pattern: /(troubleshoot|diagnos|doctor|debug)/,
            tag: "troubleshooting",
        },
    ];
...
async function main() {
    ...
    const rootAssetFiles = rootEntries
        .filter(
            (entry) => entry.isFile() && ROOT_ASSET_PATTERN.test(entry.name),
        )
        .map((entry) => path.join(repoRoot, entry.name));

    const docsAllFiles = await walkFiles(docsRoot);
    const markdownDocs = docsAllFiles.filter((filePath) =>
    ...
    for (const filePath of markdownFiles) {
        ...
        manifestEntries.push({
            ...
            sourceUrl: `https://github.com/deepakdgupta1/zeroclaw/blob/main/${relativePath}`,
        });
    }
...
main().catch((error) => {
    process.stderr.write(
        `[docs-manifest] generation failed: ${String(error)}\n`,
    );
    process.exit(1);
});
