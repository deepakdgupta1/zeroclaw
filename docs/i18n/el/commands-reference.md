...
| `uninstall` | Απεγκατάσταση του ZeroClaw από το τρέχον σύστημα με βάση τον τρόπο εγκατάστασης. |
...
- Η `uninstall` προσπαθεί πρώτα να εκτελέσει `zeroclaw service uninstall` ώστε να αφαιρεθούν οι background υπηρεσίες πριν από το binary cleanup.
- Για εγκαταστάσεις Homebrew χρησιμοποιείται το `brew uninstall zeroclaw`.
- Self-managed binaries σε `~/.cargo/bin`, `~/.local/bin` και `/usr/local/bin` αφαιρούνται απευθείας.
- Τα runtime δεδομένα διατηρούνται· χρησιμοποιήστε `--instructions` για να δείτε ποιες διαδρομές μένουν πίσω.
