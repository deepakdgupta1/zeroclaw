...
- `.github/workflows/ci-run.yml` (`CI`)
    ...
    - Hành vi bổ sung: các PR thay đổi đường dẫn CI/CD (`.github/workflows/**`, `.github/codeql/**`, `.github/connectivity/**`, `.github/release/**`, `.github/security/**`, `.github/actionlint.yaml`, `.github/dependabot.yml`, `scripts/ci/**` và tài liệu CI governance) yêu cầu ít nhất một review phê duyệt từ login trong `WORKFLOW_OWNER_LOGINS` (fallback biến repository: `deepakdgupta1,willsarg,chumyin`).
