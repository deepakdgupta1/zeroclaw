...
it pulls `ghcr.io/deepakdgupta1/zeroclaw:latest` and tags it locally before running.
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/deepakdgupta1/zeroclaw:latest`)
- `--skip-install`
- `--provider <id>`
